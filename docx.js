...
function run(text, { bold, italic, size, color, caps } = {}) {
  const props = [
    bold ? "<w:b/>" : "",
    italic ? "<w:i/>" : "",
    caps ? "<w:caps/>" : "",
...
function paragraph(runs, { before = 0, after = 120, border, align, shading } = {}) {
  ...
  const pPr = `<w:pPr><w:spacing w:before="${before}" w:after="${after}" w:line="288" w:lineRule="auto"/>${jc}${borderXml}${shadingXml}</w:pPr>`;
...
function garmentSectionXml(section) {
  const parts = [];
  parts.push(
    paragraph(run(section.label, { bold: true, size: 24, color: INK, caps: true }), {
      before: 200,
      after: 100,
  ...
  if (section.measurements.length) {
    parts.push(twoColTable(section.measurements.map((m) => [m.label, m.value]), { labelWidth: 3200, valueWidth: 7600 }));
  }
  if (section.notes) {
    parts.push(paragraph(run("Notes", { bold: true, size: 18, color: SLATE, caps: true }), { before: 120, after: 40 }));
    section.notes.split("\n").filter(Boolean).forEach((line) => {
      parts.push(paragraph(run(`•  ${line}`, { size: 20, color: INK }), { after: 40 }));
    });
  }
  return parts.join("");
}
...
function buildDocumentXml(ticket) {
  ...
  body.push(paragraph(run("J. MUESER", { bold: true, size: 40, color: INK, caps: true }), { after: 20 }));
  body.push(paragraph(run("Alterations Ticket", { size: 20, color: SLATE, caps: true }), { after: 160, border: { side: "bottom", sz: 12, color: BRASS } }));

  if (ticket.rush) {
    body.push(paragraph(run("★  RUSH", { bold: true, size: 22, color: OXBLOOD }), { after: 140 }));
  }
  ...
  body.push(
    twoColTable(
      [
        ...
        ["Tailor", ticket.tailor],
        ["Salesperson", ticket.salesperson],
        ["Due Date", ticket.dueDate],
        ["Balance", ticket.balanceDisplay || "—"],
      ],
      { labelWidth: 2400, valueWidth: 8400 },
    ),
  );

  body.push(paragraph(run(`Created ${ticket.createdDisplay}`, { size: 15, color: SLATE }), { before: 100, after: 60 }));
