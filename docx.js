...
function garmentSectionXml(section) {
  ...
  section.adjustments.forEach((line) => {
    parts.push(paragraph(run(line, { bold: true, size: 20, color: INK }), { after: 60 }));
  });
  if (section.attributes.length) {
    parts.push(twoColTable(section.attributes.map((a) => [a.label, a.value]), { labelWidth: 3200, valueWidth: 7600 }));
  }
  if (section.notes) {
...
function buildDocumentXml(ticket) {
  ...
  body.push(paragraph(run(ticket.customerName, { bold: true, size: 26, color: INK, font: FONT_DISPLAY }), { after: 200 }));
