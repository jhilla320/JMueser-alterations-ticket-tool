...
export async function fetchDriveFolders(token, parentId, { query = "", onProgress } = {}) {
...
export function chooseDriveFolder(token, dom) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const renderFolderRows = (folders, totalCount) => {
      if (!folders.length) {
        list.innerHTML = '<p class="drive-folder-empty">No folders found here.</p>';
        return;
      }
      const rows = folders
        .slice(0, RENDER_CAP)
    ...
    const applyFilter = () => {
      ...
      renderFolderRows(matches, matches.length);
    };

    // Loads every subfolder of the current folder (paginating past Drive's
    // 1000-per-page limit as needed) so browsing and search both see the
      ...
      try {
        const folders = await fetchDriveFolders(token, folder.id, {
          onProgress: (count) => {
            ...
            list.innerHTML = `<p class="drive-folder-empty">Loading folders… (${count.toLocaleString()} so far)</p>`;
          },
        });
        if (myLoad !== loadToken) return; // superseded by a newer navigation
        allFolders = folders;
        applyFilter(); // respects a search term typed while this was loading
      } catch (err) {
        fail(err);
      }
    };
    ...
    const handleCreateFolder = async () => {
      ...
      try {
        const created = await createDriveFolder(token, currentFolder().id, name);
