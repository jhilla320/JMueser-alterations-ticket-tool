...
export async function appendTicketRow(token, ledgerId, ticket) {
  ...
  await sheetsApi(
    token,
    `${ledgerId}/values/${encodeURIComponent(`${SHEET_TAB}!A:L`)}:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`,
    { method: "POST", body: JSON.stringify({ values: [row] }) },
  );
}

export async function listTickets(token, ledgerId) {
  const data = await sheetsApi(token, `${ledgerId}/values/${encodeURIComponent(`${SHEET_TAB}!A2:L`)}`);
...
export async function updateTicketStatus(token, ledgerId, rowNumber, status) {
  await sheetsApi(token, `${ledgerId}/values/${encodeURIComponent(`${SHEET_TAB}!I${rowNumber}`)}?valueInputOption=RAW`, {
    method: "PUT",
