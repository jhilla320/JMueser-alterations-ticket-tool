...
export async function appendTicketRow(token, ledgerId, ticket) {
  ...
  const result = await sheetsApi(
    token,
    `${ledgerId}/values/${encodeURIComponent(`${SHEET_TAB}!A:N`)}:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`,
...
export async function listTickets(token, ledgerId) {
  const data = await sheetsApi(token, `${ledgerId}/values/${encodeURIComponent(`${SHEET_TAB}!A2:N`)}`);
