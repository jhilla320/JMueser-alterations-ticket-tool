...
import { getOrCreateLedger, appendTicketRow, updateTicketRecord, listTickets, updateTicketStatus, deleteTicketRow } from "./sheets.js";
...
function setEditingTicket(ticket) {
  ...
  if (ticket) {
    ...
    driveSaveBtn.textContent = "✂ Save to Google";
  }
}

editingBannerExit.addEventListener("click", () => setEditingTicket(null));
    ...
    .map((ticket) => {
      ...
      return `
        <tr data-row="${ticket.rowNumber}" data-ticket-id="${escapeHtml(ticket.id)}">
          ...
          <td data-label="Ticket"><span class="cell-value ticket-actions">${linkCell}<button type="button" class="btn-ghost reopen-btn" data-row="${ticket.rowNumber}">Reopen</button><button type="button" class="btn-danger delete-btn" data-row="${ticket.rowNumber}">Delete</button></span></td>
        </tr>`;
    })
    .join("");
}
