...
function renderTicketMarkup(ticketData) {
  const sectionHtml = ticketData.garmentSections.map((section) => {
    ...
    const attributeHtml = section.attributes.map((a) => `<p><strong>${escapeHtml(a.label)}:</strong> ${escapeHtml(a.value)}</p>`).join("");
  ...
  return `
    ...
    <div class="ticket-hero">
      ...
      <p class="hero-due-date"><span class="hero-due-label">Due</span>${escapeHtml(ticketData.dueDate)}</p>
