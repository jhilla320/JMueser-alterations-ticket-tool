...
function buildGarmentSections(items = garmentItems) {
  ...
  const buildJacketSection = (entry, label) => {
    ...
    if (buttonParts.length) measurements.push({ label: "Sleeve Buttons", value: buttonParts.join(" · ") });
  ...
  const buildTrouserSection = (entry, label) => {
    ...
    if ((entry.trouserTotalLength || "").trim()) measurements.push({ label: "Total Inseam", value: `${entry.trouserTotalLength} cm` });
