...
function summarizeSingleItem(item) {
  const parts = [item.size, item.description].map((v) => (v || "").trim()).filter(Boolean);
      ...
      return `
        <section class="repeat-item" data-uid="${item._uid}">
          ...
          <div class="garment-field size-row">
            <label for="size-${item._uid}">Size</label>
            <input id="size-${item._uid}" type="text" placeholder="e.g. 40R" data-uid="${item._uid}" data-field="size" value="${escapeHtml(item.size)}" />
          </div>
          <div class="garment-field">
            <label for="description-${item._uid}">Description</label>
            <input id="description-${item._uid}" type="text" data-uid="${item._uid}" data-field="description" value="${escapeHtml(item.description)}" />
          </div>
          ${measurementBlock}
...
function normalizeGarmentItems(state) {
  ...
  items = items.map((item) => ({ ...item, _uid: uidCounter++ }));
