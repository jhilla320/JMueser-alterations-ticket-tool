...
function addGarmentItem(garmentType) {
  // Minimize whatever's already filled in, so the newly added tile is the
  // one clear focal point — matches "click the button, get a fresh tile"
  // rather than a wall of expanded tiles piling up.
  garmentItems.forEach((it) => {
    if (hasGarmentData(it)) collapsedItemIds.add(String(it._uid));
  });
