...
import { getOrCreateLedger, appendTicketRow, listTickets, updateTicketStatus } from "./sheets.js";
...
const statusFilterWrap = el("statusFilterWrap");
const statusFilterBtn = el("statusFilterBtn");
const statusFilterPanel = el("statusFilterPanel");
const statusFilterOptions = el("statusFilterOptions");
const statusFilterAll = el("statusFilterAll");
const statusFilterNone = el("statusFilterNone");
const ticketTailorFilter = el("ticketTailorFilter");
const ticketSalespersonFilter = el("ticketSalespersonFilter");
...
/* -------------------------------- ticket log --------------------------------- */
let selectedStatuses = new Set(); // empty = no status filter (all statuses)

function populateFilterOptions() {
  statusFilterOptions.innerHTML = TICKET_STATUSES.map(
    (status) => `
  ...
  ticketSalespersonFilter.innerHTML =
    '<option value="">All salespeople</option>' +
    SALESPEOPLE.map((name) => `<option value="${escapeHtml(name)}">${escapeHtml(name)}</option>`).join("");
}

function updateStatusFilterButtonLabel() {
  if (selectedStatuses.size === 0 || selectedStatuses.size === TICKET_STATUSES.length) {
    statusFilterBtn.textContent = "All statuses";
  } else if (selectedStatuses.size === 1) {
    statusFilterBtn.textContent = [...selectedStatuses][0];
  } else {
    statusFilterBtn.textContent = `${selectedStatuses.size} statuses`;
  }
}

statusFilterBtn.addEventListener("click", () => {
  statusFilterPanel.hidden = !statusFilterPanel.hidden;
});

document.addEventListener("click", (event) => {
  if (!statusFilterWrap.contains(event.target)) statusFilterPanel.hidden = true;
});

statusFilterOptions.addEventListener("change", (event) => {
  const checkbox = event.target.closest('input[type="checkbox"]');
  if (!checkbox) return;
  if (checkbox.checked) selectedStatuses.add(checkbox.value);
  else selectedStatuses.delete(checkbox.value);
  updateStatusFilterButtonLabel();
  renderTicketLog();
});

statusFilterAll.addEventListener("click", () => {
  selectedStatuses = new Set(TICKET_STATUSES);
  statusFilterOptions.querySelectorAll('input[type="checkbox"]').forEach((cb) => { cb.checked = true; });
  updateStatusFilterButtonLabel();
  renderTicketLog();
});
...
statusFilterNone.addEventListener("click", () => {
  ...
  statusFilterOptions.querySelectorAll('input[type="checkbox"]').forEach((cb) => { cb.checked = false; });
  updateStatusFilterButtonLabel();
  renderTicketLog();
});
...
function renderTicketLog() {
  ...
  const tailorFilter = ticketTailorFilter.value;
  const salespersonFilter = ticketSalespersonFilter.value;
  ...
  const filtered = ticketCache.filter((ticket) => {
    ...
    if (selectedStatuses.size && !selectedStatuses.has(ticket.status)) return false;
    if (tailorFilter && ticket.tailor !== tailorFilter) return false;
    if (salespersonFilter && ticket.salesperson !== salespersonFilter) return false;
    ...
    .map((ticket) => {
      ...
      return `
        <tr data-row="${ticket.rowNumber}" data-ticket-id="${escapeHtml(ticket.id)}">
          ...
          <td data-label="Ticket"><span class="cell-value">${linkCell} · <button type="button" class="btn-ghost reopen-btn" data-row="${ticket.rowNumber}">Reopen</button></span></td>
        </tr>`;
    })
    .join("");
}
...
ticketTailorFilter.addEventListener("change", renderTicketLog);
ticketSalespersonFilter.addEventListener("change", renderTicketLog);
...
ticketLogBody.addEventListener("click", (event) => {
...
downloadBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    const filename = `${safeName}_${datePart}_${formatFileTime(new Date(ticketData.createdAt))}.docx`;
