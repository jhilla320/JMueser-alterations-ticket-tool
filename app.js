import { JACKET_SIZES, TROUSER_SIZES, SHIRT_SIZES, TICKET_STATUSES } from "./config.js";
...
const ticketStatusFilter = el("ticketStatusFilter");
...
function renderTicketLog() {
  ...
  if (!filtered.length) {
    ticketLogBody.innerHTML = `<tr><td colspan="7" class="log-status">No tickets match yet.</td></tr>`;
    return;
  }
    ...
    .map((ticket) => {
      ...
      return `
        <tr data-row="${ticket.rowNumber}" data-ticket-id="${escapeHtml(ticket.id)}">
          <td>${escapeHtml(ticket.customerName)}${ticket.rush ? '<span class="rush-pill">Rush</span>' : ""}</td>
          <td>${escapeHtml(ticket.tailor)}</td>
          <td>${escapeHtml(ticket.salesperson)}</td>
          <td>${escapeHtml(ticket.dueDate)}</td>
          <td>${escapeHtml(ticket.balance || "—")}</td>
          <td><select class="status-select" data-row="${ticket.rowNumber}" data-status="${escapeHtml(ticket.status)}">${statusOptions}</select></td>
          <td>${linkCell} · <button type="button" class="btn-ghost reopen-btn" data-row="${ticket.rowNumber}">Reopen</button></td>
        </tr>`;
    })
    .join("");
}
...
ticketStatusFilter.addEventListener("change", renderTicketLog);
...
ticketLogBody.addEventListener("change", async (event) => {
  ...
  try {
    const token = await auth.getValidToken();
    const ledgerId = await getOrCreateLedger(token);
    await updateTicketStatus(token, ledgerId, rowNumber, select.value);
